...
1. 첫 화면은 안내 문구와 HUD 없이 TEAM 5 은빛 팩만 보여 주며, 팩을 곧바로 선택합니다.
...
6. 공개 화면 좌측 상단의 `← 이전으로`는 남은 팩이 있으면 팩 선택으로, 마지막 카드면 컬렉션으로 이동합니다.
7. 컬렉션 좌측 상단의 `← 팩 선택`은 모든 공개 상태를 초기화하고 처음 팩 선택 화면으로 돌아갑니다. 세 카드는 겹치지 않고 가로로 정렬합니다.
8. 터널 우측 아래 버튼이나 `Escape` 키로 긴 연출을 즉시 건너뛸 수 있습니다.
9. `prefers-reduced-motion` 환경에서는 애니메이션 시간을 최소화합니다.

## 실제 카드 자산과 실루엣 정렬

원본 카드는 보존하고, 웹에서는 카드 외부 배경을 보이지 않게 처리한 파생 PNG를 사용합니다.

| 팀원 | 원본 파일 | 원본 비율 | 홈페이지용 파생 파일 |
| --- | --- | --- | --- |
| 엄태웅 | `assets/바르샤태웅카드.png` | 1086 × 1448 | `assets/cards/taewoong-card-cutout-v2.png` |
| 김영광 | `assets/첼시영광카드.png` | 1672 × 941 | `assets/cards/youngkwang-card-cutout-v2.png` |
| 한예진 | `assets/유벤예진카드.png` | 1122 × 1402 | `assets/cards/yejin-card-cutout-v2.png` |

`scripts/normalize-cards.ps1`가 각 원본의 실제 카드 영역을 3:4 JPEG로 맞춥니다. 현재 화면은 카드 외부를 실제 투명 알파로 분리한 `*-cutout-v2.png`를 사용합니다. 카드 내부의 얼굴, 이름, KAI, MBTI, 태그와 하단 TEAM 5는 유지하고, 원본 파일은 수정하지 않았습니다. 빛과 그림자는 PNG의 투명 외곽선을 따라 적용됩니다.
...
| `styles.css` | 경기장 공간감, 터널 원근, 플래시, 카드 실루엣 그림자, 반응형 레이아웃 |
...
- 터널 진입 후 약 1.6초 뒤 첫 힌트를 시작합니다.
- 힌트는 장당 약 3초씩 유지됩니다.
- 마지막 힌트 약 0.16초 뒤 최종 백색 점화와 카드 공개를 시작합니다.
- 전체 강제 대기 시간은 약 11초이며 언제든 건너뛸 수 있습니다.

타이밍 상수는 `script.js`의 `openPack()`과 `finishReveal()`에서 수정합니다. 시각 애니메이션은 `styles.css`의 `white-burst`, `tunnel-rush`, `hint-arrive`, `card-impact` 키프레임입니다.
